...
import { TableEntity, TableEntityRow } from './index.js';
import { pricePlanTableSchema } from '../../entities/price-plans.js';
...
export const TableEntityDemo: React.FC = () => {
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h2>Price Plans Table</h2>
      <TableEntity
        data={data}
        columns={pricePlanTableSchema}
        onEdit={handleEdit}
      >
        {(item) => (
          <TableEntityRow key={item?.id} item={item} />
        )}
      </TableEntity>
    </div>
  );
